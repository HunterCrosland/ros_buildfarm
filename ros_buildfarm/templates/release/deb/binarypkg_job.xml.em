...
@(SNIPPET(
    ...
    script='\n'.join([
        ...
        'if [ -f $WORKSPACE/binarydeb ] ; then sudo chmod -R u+w $WORKSPACE/binarydeb ; fi',
...
@(SNIPPET(
    ...
    script='\n'.join([
        ...
        'python3 -u $WORKSPACE/ros_buildfarm/scripts/misc/docker_pull_baseimage.py',
        'docker build --force-rm -t --no-cache binarydeb_build.%s_%s_%s_%s_%s .' % (rosdistro_name, os_name, os_code_name, arch, pkg_name),
        'echo "# END SECTION"',
        '',
...
@(SNIPPET(
    ...
    script='\n'.join([
        ...
        '# ensure to have write permission before trying to delete the folder',
        'sudo chmod -R u+w $WORKSPACE/binarydeb',
